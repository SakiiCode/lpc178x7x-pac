...
pub type R = crate::R<StaticwaitrdSpec>;
#[doc = "Register `STATICWAITRD%s` writer"]
pub type W = crate::W<StaticwaitrdSpec>;
...
pub type WaitrdR = crate::FieldReader;
#[doc = "Field `WAITRD` writer - Non-page mode read wait states or asynchronous page mode read first access wait state. Non-page mode read or asynchronous page mode read, first read only: 0x0 - 0x1E = (n + 1) CCLK cycles for read accesses. For non-sequential reads, the wait state time is (WAITRD + 1) x tCCLK. 0x1F = 32 CCLK cycles for read accesses (POR reset value)."]
pub type WaitrdW<'a, REG> = crate::FieldWriter<'a, REG, 5>;
impl R {
    #[doc = "Bits 0:4 - Non-page mode read wait states or asynchronous page mode read first access wait state. Non-page mode read or asynchronous page mode read, first read only: 0x0 - 0x1E = (n + 1) CCLK cycles for read accesses. For non-sequential reads, the wait state time is (WAITRD + 1) x tCCLK. 0x1F = 32 CCLK cycles for read accesses (POR reset value)."]
    #[inline(always)]
    pub fn waitrd(&self) -> WaitrdR {
        WaitrdR::new((self.bits & 0x1f) as u8)
    }
}
impl W {
    #[doc = "Bits 0:4 - Non-page mode read wait states or asynchronous page mode read first access wait state. Non-page mode read or asynchronous page mode read, first read only: 0x0 - 0x1E = (n + 1) CCLK cycles for read accesses. For non-sequential reads, the wait state time is (WAITRD + 1) x tCCLK. 0x1F = 32 CCLK cycles for read accesses (POR reset value)."]
    #[inline(always)]
    pub fn waitrd(&mut self) -> WaitrdW<'_, StaticwaitrdSpec> {
        WaitrdW::new(self, 0)
    }
}
...
pub struct StaticwaitrdSpec;
impl crate::RegisterSpec for StaticwaitrdSpec {
    type Ux = u32;
}
#[doc = "`read()` method returns [`staticwaitrd::R`](R) reader structure"]
impl crate::Readable for StaticwaitrdSpec {}
#[doc = "`write(|w| ..)` method takes [`staticwaitrd::W`](W) writer structure"]
impl crate::Writable for StaticwaitrdSpec {
    type Safety = crate::Unsafe;
}
#[doc = "`reset()` method sets STATICWAITRD%s to value 0x1f"]
impl crate::Resettable for StaticwaitrdSpec {
